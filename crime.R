...
ggplot(count(top_crime, time, OFNS_DESC)) +
  aes(time, n) +
  geom_point(alpha = .1) +
  theme_bw() +
  geom_smooth(formula = expnd(4), color = "red",
...
ggplot(count(na.omit(top_crime), 
             time, 
             quarter,
             OFNS_DESC)) +
  aes(time, n) +
...
#polygons
shape <- readOGR("Downloads/shape/", "shape")
plot(x = shape)
